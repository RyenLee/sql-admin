use crate::state::use_app_state;
use leptos::prelude::*;

#[component]
pub fn QuickStartPage() -> impl IntoView {
    let app_state = use_app_state();
    let dark_mode = app_state.dark_mode;

    view! {
        <div class=move || {
            if dark_mode.get() {
                "min-h-full bg-gray-900 p-6"
            } else {
                "min-h-full bg-white p-6"
            }
        }>
            <div class="max-w-4xl mx-auto">
                <div class="flex items-center mb-6">
                    <h1 class=move || {
                        if dark_mode.get() {
                            "text-2xl font-bold text-white"
                        } else {
                            "text-2xl font-bold text-gray-900"
                        }
                    }>"Quick Start"</h1>
                </div>

                <div class=move || {
                    if dark_mode.get() {
                        "bg-gray-800 rounded-lg p-6"
                    } else {
                        "bg-gray-50 rounded-lg p-6"
                    }
                }>
                    <div class="space-y-4">
                        <div class=move || {
                            if dark_mode.get() {
                                "bg-gray-700 rounded-lg p-4"
                            } else {
                                "bg-white rounded-lg p-4 border"
                            }
                        }>
                            <div class="flex items-start">
                                <span class="text-blue-500 font-bold mr-3">"1."</span>
                                <div>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-300 font-medium mb-1"
                                        } else {
                                            "text-gray-700 font-medium mb-1"
                                        }
                                    }>"Connect to a Database"</p>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-400 text-sm"
                                        } else {
                                            "text-gray-500 text-sm"
                                        }
                                    }>"Click File > New Connection to add a new database connection."</p>
                                </div>
                            </div>
                        </div>
                        <div class=move || {
                            if dark_mode.get() {
                                "bg-gray-700 rounded-lg p-4"
                            } else {
                                "bg-white rounded-lg p-4 border"
                            }
                        }>
                            <div class="flex items-start">
                                <span class="text-blue-500 font-bold mr-3">"2."</span>
                                <div>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-300 font-medium mb-1"
                                        } else {
                                            "text-gray-700 font-medium mb-1"
                                        }
                                    }>"Create a Query"</p>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-400 text-sm"
                                        } else {
                                            "text-gray-500 text-sm"
                                        }
                                    }>"Click File > New Query to open a new SQL editor tab."</p>
                                </div>
                            </div>
                        </div>
                        <div class=move || {
                            if dark_mode.get() {
                                "bg-gray-700 rounded-lg p-4"
                            } else {
                                "bg-white rounded-lg p-4 border"
                            }
                        }>
                            <div class="flex items-start">
                                <span class="text-blue-500 font-bold mr-3">"3."</span>
                                <div>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-300 font-medium mb-1"
                                        } else {
                                            "text-gray-700 font-medium mb-1"
                                        }
                                    }>"Execute SQL"</p>
                                    <p class=move || {
                                        if dark_mode.get() {
                                            "text-gray-400 text-sm"
                                        } else {
                                            "text-gray-500 text-sm"
                                        }
                                    }>"Write your SQL query and click the Run button or press Ctrl+Enter."</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    }
}
